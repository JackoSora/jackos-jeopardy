...
fn test_validate_team_action() {
    ...
    let select_action = GameAction::SelectClue { clue: (0, 0), team_id };
    ...
    let invalid_select = GameAction::SelectClue { clue: (0, 0), team_id: other_team_id };
...
fn test_get_available_actions() {
    ...
    assert!(actions.iter().any(|a| matches!(a, GameAction::AddTeam { .. })));
    ...
    assert!(actions.iter().any(|a| matches!(a, GameAction::SelectClue { .. })));
}