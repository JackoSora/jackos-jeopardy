use super::*;
use crate::game::{GameAction, GameActionResult, GameEffect, FlashType};
...
fn test_add_team_action_handler() {
    ...
    let action = GameAction::AddTeam { name: "New Team".to_string() };
...
fn test_start_game_validation() {
    ...
    let _ = engine.handle_action(GameAction::AddTeam { name: "Team 1".to_string() });
    let result = engine.handle_action(GameAction::StartGame);
    assert!(result.is_ok());
}
...
fn test_clue_selection_validation() {
    ...
    let valid_action = GameAction::SelectClue { clue: (0, 0), team_id };
    ...
    let invalid_action = GameAction::SelectClue { clue: (10, 10), team_id };
...
fn test_answer_correct_effects() {
    ...
    if let Ok(GameActionResult::StateChanged { effects, .. }) = result {
        ...
        let has_score_effect = effects.iter().any(|e| matches!(e, GameEffect::ScoreChanged { .. }));
        let has_flash_effect = effects.iter().any(|e| matches!(e, GameEffect::FlashEffect { effect_type: FlashType::Correct }));
...
fn test_steal_attempt_correct() {
    ...
    // Set up steal phase
    let _ = engine.handle_action(GameAction::SelectClue { clue, team_id: owner_team_id });
    let _ = engine.handle_action(GameAction::AnswerIncorrect { clue, team_id: owner_team_id });
    
    // Attempt steal (correct)
    let action = GameAction::StealAttempt { clue, team_id: stealing_team_id, correct: true };
    let result = engine.handle_action(action);
