...
impl GameActionHandler {
    ...
    pub fn handle(&self, state: &mut crate::game::state::GameState, action: GameAction) -> Result<GameActionResult, GameError> {
        match action {
            GameAction::AddTeam { name } => self.handle_add_team(state, name),
            GameAction::StartGame => self.handle_start_game(state),
            GameAction::SelectClue { clue, team_id } => self.handle_select_clue(state, clue, team_id),
            GameAction::AnswerCorrect { clue, team_id } => self.handle_answer_correct(state, clue, team_id),
            GameAction::AnswerIncorrect { clue, team_id } => self.handle_answer_incorrect(state, clue, team_id),
            GameAction::StealAttempt { clue, team_id, correct } => self.handle_steal_attempt(state, clue, team_id, correct),
            GameAction::CloseClue { clue, next_team_id } => self.handle_close_clue(state, clue, next_team_id),
            GameAction::ReturnToConfig => self.handle_return_to_config(state),
        }
    }

    fn handle_add_team(&self, state: &mut crate::game::state::GameState, name: String) -> Result<GameActionResult, GameError> {
        ...
        Ok(GameActionResult::Success { new_phase: state.phase.clone() })
    }

    fn handle_start_game(&self, state: &mut crate::game::state::GameState) -> Result<GameActionResult, GameError> {
        ...
        let new_phase = PlayPhase::Selecting { team_id: first_team_id };
        state.phase = new_phase.clone();

        Ok(GameActionResult::Success { new_phase })
    }

    fn handle_select_clue(&self, state: &mut crate::game::state::GameState, clue: (usize, usize), team_id: u32) -> Result<GameActionResult, GameError> {
        ...
        let new_phase = PlayPhase::Showing { clue, owner_team_id: team_id };
        state.phase = new_phase.clone();

        Ok(GameActionResult::Success { new_phase })
    }

    fn handle_answer_correct(&self, state: &mut crate::game::state::GameState, clue: (usize, usize), team_id: u32) -> Result<GameActionResult, GameError> {
        ...
        if let Some(category) = state.board.categories.get_mut(clue.0) {
            if let Some(c) = category.clues.get_mut(clue.1) {
                ...
                if self.scoring.award_points(&mut state.teams, team_id, c.points as i32) {
                    effects.push(GameEffect::ScoreChanged { team_id, delta: c.points as i32 });
                }
            }
        }

        effects.push(GameEffect::FlashEffect { effect_type: FlashType::Correct });

        let new_phase = PlayPhase::Resolved { clue, next_team_id: team_id };
        state.phase = new_phase.clone();

        Ok(GameActionResult::StateChanged { new_phase, effects })
    }

    fn handle_answer_incorrect(&self, state: &mut crate::game::state::GameState, clue: (usize, usize), team_id: u32) -> Result<GameActionResult, GameError> {
        ...
        if let Some(category) = state.board.categories.get(clue.0) {
            if let Some(c) = category.clues.get(clue.1) {
                if self.scoring.deduct_points(&mut state.teams, team_id, c.points as i32) {
                    effects.push(GameEffect::ScoreChanged { team_id, delta: -(c.points as i32) });
                }
            }
        }

        effects.push(GameEffect::FlashEffect { effect_type: FlashType::Incorrect });
        ...
        let new_phase = PlayPhase::Steal { clue, queue, current, owner_team_id: team_id };
        state.phase = new_phase.clone();

        Ok(GameActionResult::StateChanged { new_phase, effects })
    }

    fn handle_steal_attempt(&self, state: &mut crate::game::state::GameState, clue: (usize, usize), team_id: u32, correct: bool) -> Result<GameActionResult, GameError> {
        let action = GameAction::StealAttempt { clue, team_id, correct };
        if !self.rules.validate_team_action(state, team_id, &action) {
            return Err(GameError::InvalidAction {
        ...
        if let PlayPhase::Steal { queue, current, owner_team_id, .. } = &mut state.phase {
            ...
            if correct {
                ...
                if let Some(next_team) = queue.pop_front() {
                    *current = next_team;
                    Ok(GameActionResult::StateChanged { new_phase: state.phase.clone(), effects })
                } else {
                    ...
                    let new_phase = PlayPhase::Resolved { clue, next_team_id: *owner_team_id };
                    state.phase = new_phase.clone();
    ...
    fn handle_close_clue(&self, state: &mut crate::game::state::GameState, clue: (usize, usize), next_team_id: u32) -> Result<GameActionResult, GameError> {
        ...
        let new_phase = PlayPhase::Selecting { team_id: next_team_id };
        state.phase = new_phase.clone();

        Ok(GameActionResult::Success { new_phase })
    }

    fn handle_return_to_config(&self, _state: &mut crate::game::state::GameState) -> Result<GameActionResult, GameError> {
        // This will be handled at the app level, not at the game state level
        Ok(GameActionResult::Success { new_phase: PlayPhase::Finished })
    }
}