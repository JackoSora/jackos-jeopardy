...
pub fn paint_subtle_modal_background(painter: &egui::Painter, rect: egui::Rect) {
    ...
    let border_glow = GlowConfig::new(
        Palette::SUBTLE_TEAL,
        0.12, // Reduced from 0.3 to 0.12
        15.0,
    );
    paint_glow_rect(painter, rect.shrink(50.0), 12.0, border_glow);

    // Add corner accents with reduced brightness
    let corner_size = 35.0;
    let corner_color = adjust_brightness(Palette::SUBTLE_TEAL, 0.8);

    // Top-left corner
    let tl_corner = egui::Rect::from_min_size(rect.min, egui::vec2(corner_size, corner_size));
    painter.rect_stroke(tl_corner, 0.0, egui::Stroke::new(2.0, corner_color));
    ...
    painter.rect_stroke(tr_corner, 0.0, egui::Stroke::new(2.0, corner_color));
    ...
    painter.rect_stroke(bl_corner, 0.0, egui::Stroke::new(2.0, corner_color));
    ...
    painter.rect_stroke(br_corner, 0.0, egui::Stroke::new(2.0, corner_color));

    // Add subtle layering effect for visual depth
    let inner_rect = rect.shrink(8.0);
    let layer_color = adjust_brightness(Palette::BG_DARK, 1.15);
    painter.rect_stroke(inner_rect, 2.0, egui::Stroke::new(1.0, layer_color));
}
