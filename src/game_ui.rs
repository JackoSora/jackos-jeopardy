    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        match gs.phase {
            ...
            PlayPhase::Selecting { team_id } => {
                ...
                let header_h = 40.0; let cell_h = 70.0;
                ui.horizontal(|ui| {
                    ...
                    for cat in &gs.board.categories {
                        ...
                        painter.text(rect.center(), egui::Align2::CENTER_CENTER, &cat.name, egui::FontId::proportional(18.0), Palette::CYAN);
                        painter.line_segment([rect.left_bottom(), rect.right_bottom()], egui::Stroke::new(2.0, Palette::MAGENTA));
                    }
                });
                for r in 0..rows { ui.horizontal(|ui| { ui.set_width(available.x); for (ci, cat) in gs.board.categories.iter().enumerate() { let clue = &cat.clues[r]; let (rect, response) = ui.allocate_exact_size(egui::vec2(cell_w, cell_h), egui::Sense::click()); let painter = ui.painter_at(rect); let base = if clue.solved { Palette::BG_PANEL } else { Palette::BG_ACTIVE }; painter.rect_filled(rect, 6.0, base); painter.rect_stroke(rect.expand(1.0), 6.0, egui::Stroke::new(2.0, Palette::CYAN)); painter.text(rect.center(), egui::Align2::CENTER_CENTER, format!("{}", clue.points), egui::FontId::proportional(20.0), Palette::TEXT); if !clue.solved && response.clicked() { requested_phase = Some(PlayPhase::Showing { clue: (ci, r), owner_team_id: team_id }); } } }); }
            }
        ...
        if let Some((kind, start)) = flash { let elapsed = start.elapsed(); let duration = Duration::from_millis(700); if elapsed < duration { let t = (elapsed.as_secs_f32() / duration.as_secs_f32()).clamp(0.0, 1.0); let alpha = (220.0 * (1.0 - t)) as u8; let color = match kind { AnswerFlash::Correct => egui::Color32::from_rgba_unmultiplied(0,255,170,alpha), AnswerFlash::Incorrect => egui::Color32::from_rgba_unmultiplied(255,0,150,alpha) }; let rect = ui.max_rect(); ui.painter().rect_filled(rect, 0.0, color); let inset = 20.0 * (1.0 + 0.2 * (1.0 - t)); ui.painter().rect_stroke(rect.shrink(inset), 8.0, egui::Stroke::new(6.0, color)); ui.ctx().request_repaint(); ui.memory_mut(|m| m.data.insert_temp(flash_id, Some((kind, start)))); } else { ui.memory_mut(|m| m.data.remove::<Option<(AnswerFlash, Instant)>>(flash_id)); } } else { ui.memory_mut(|m| m.data.remove::<Option<(AnswerFlash, Instant)>>(flash_id)); }
    });
    next_mode
}
...
fn draw_showing_overlay(ctx: &egui::Context, gs: &mut GameState, clue: (usize, usize), owner_team_id: u32, flash: &mut Option<(AnswerFlash, Instant)>, requested_phase: &mut Option<PlayPhase>) {
    ...
    egui::Area::new("question_full_overlay".into()).order(egui::Order::Foreground).movable(false).interactable(true).fixed_pos(screen.min).show(ctx, |ui| {
        ...
        let (question, points) = gs.board.categories.get(clue.0).and_then(|cat| cat.clues.get(clue.1)).map(|c| (c.question.clone(), c.points)).unwrap_or_default();
        ui.allocate_ui_with_layout(rect.size(), egui::Layout::top_down(egui::Align::Center), |ui| { ui.add_space(32.0); ui.heading(egui::RichText::new(format!("{} pts", points)).color(Palette::CYAN)); ui.add_space(16.0); let wrap_width = rect.width() * 0.9; let label = egui::Label::new(egui::RichText::new(question).size(28.0)).wrap(true).truncate(false); ui.add_sized([wrap_width, 0.0], label); });
        ...
        ui.allocate_ui_at_rect(bottom_rect, |ui| { ui.with_layout(egui::Layout::centered_and_justified(egui::Direction::LeftToRight), |ui| { ui.set_width(bottom_rect.width()); ui.horizontal(|ui| { let correct_btn = egui::Button::new(egui::RichText::new("Correct").strong().color(egui::Color32::BLACK)).fill(Palette::CYAN).min_size(egui::vec2(160.0,44.0)); if ui.add(correct_btn).clicked() { if let Some(c) = gs.board.categories.get_mut(clue.0).and_then(|cat| cat.clues.get_mut(clue.1)) { c.revealed = true; c.solved = true; if let Some(team) = gs.teams.iter_mut().find(|t| t.id == owner_team_id) { team.score += c.points as i32; } } *flash = Some((AnswerFlash::Correct, Instant::now())); *requested_phase = Some(PlayPhase::Resolved { clue, next_team_id: owner_team_id }); } ui.add_space(24.0); let incorrect_btn = egui::Button::new(egui::RichText::new("Incorrect").strong().color(egui::Color32::WHITE)).fill(Palette::MAGENTA).min_size(egui::vec2(160.0,44.0)); if ui.add(incorrect_btn).clicked() { let mut others: Vec<u32> = gs.teams.iter().filter(|t| t.id != owner_team_id).map(|t| t.id).collect(); let mut rng = rand::thread_rng(); others.as_mut_slice().shuffle(&mut rng); let mut queue = std::collections::VecDeque::from(others); let current = queue.pop_front().unwrap_or(owner_team_id); if let Some(cat) = gs.board.categories.get(clue.0) { if let Some(c) = cat.clues.get(clue.1) { if let Some(team) = gs.teams.iter_mut().find(|t| t.id == owner_team_id) { team.score -= c.points as i32; } } } *flash = Some((AnswerFlash::Incorrect, Instant::now())); *requested_phase = Some(PlayPhase::Steal { clue, queue, current, owner_team_id }); } }); }); }); });
}
...
fn draw_resolved_overlay(ctx: &egui::Context, gs: &mut GameState, clue: (usize, usize), next_team_id: u32, requested_phase: &mut Option<PlayPhase>) {
    ...
    egui::Area::new("resolved_full_overlay".into()).order(egui::Order::Foreground).movable(false).interactable(true).fixed_pos(screen.min).show(ctx, |ui| {
        ...
        ui.allocate_ui_with_layout(rect.size(), egui::Layout::top_down(egui::Align::Center), |ui| { ui.add_space(32.0); ui.heading(egui::RichText::new(format!("{} pts", points)).color(Palette::CYAN)); ui.add_space(12.0); let wrap_width = rect.width() * 0.9; let q_label = egui::Label::new(egui::RichText::new(question).size(24.0)).wrap(true).truncate(false); ui.add_sized([wrap_width, 0.0], q_label); ui.add_space(8.0); let a_label = egui::Label::new(egui::RichText::new(format!("Answer: {}", answer)).color(Palette::MAGENTA).size(22.0)).wrap(true).truncate(false); ui.add_sized([wrap_width, 0.0], a_label); });
        let bottom_h = 90.0; let bottom_rect = egui::Rect::from_min_size(egui::pos2(rect.left(), rect.bottom() - bottom_h - 8.0), egui::vec2(rect.width(), bottom_h));
        ui.allocate_ui_at_rect(bottom_rect, |ui| { ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| { ui.set_width(bottom_rect.width()); ui.horizontal_centered(|ui| { let close_btn = egui::Button::new(egui::RichText::new("Close").strong().color(egui::Color32::BLACK)).fill(Palette::CYAN).min_size(egui::vec2(200.0,48.0)); if ui.add(close_btn).clicked() { *requested_phase = Some(PlayPhase::Selecting { team_id: next_team_id }); ui.ctx().request_repaint(); } }); }); });
    });
}
